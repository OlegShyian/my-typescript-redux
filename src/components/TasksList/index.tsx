...
const TasksList: React.FC = () => {
    ...
    useEffect(() => {
        if (outputTasks.length) {
            const arr = outputTasks.filter((_task: ITask, ind: number) => ind >= (currentPage - 1) * tasksLimit && ind < currentPage * tasksLimit);
    ...
    const handleChangeTaskChecked = (id: number) => {
        const editedTasks = user.tasks.map((task: ITask) => (task.id === id ? { ...task, checked: !task.checked } : task));
                ...
                {outputTasks.map((task: ITask, ind: number) =>
                    ind >= (currentPage - 1) * tasksLimit && ind < currentPage * tasksLimit ? (
                        <li key={ind} className="list__titles">
                            <div>
                                <input type="checkbox" checked={task.checked} onChange={() => handleChangeTaskChecked(task.id)} />
                            </div>
                            <div>{ind + 1}</div>
                            <div className="second__element">{task.name}</div>
                            <div>{task.status}</div>
                            <div>
                                <button onClick={() => openCurenTask(task.id)}>Edit</button>
                            </div>
                            <div>
                                <button onClick={() => removeCurrentTask(task.id)}>Remove</button>
                            </div>
                        </li>
                    ) : null
                )}
